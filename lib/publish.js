...
async function publishChartUsingCr(chartPath, crConfigPath, context) {
    ...
    const repoParts = /https:\/\/github.com(\/|:)([^/]+)\/([^\.]+)/.exec(context.options.repositoryUrl)
    if (!repoParts) {
        throw new Error("Did not match github. Only github is supported by chart-releaser")
    }
    const owner = repoParts[2]
    const project = repoParts[3]
